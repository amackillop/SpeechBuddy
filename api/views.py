...
from api.capstoneModules.audioFunctions import convertToFLAC
...
def googleCall(request):
    if request.method == 'POST':
        ...
        res = googleApiCall(ROOT+'/SpeechBuddy/audio/output_flac.flac')
        
        # Pitch Tracking
#        url = 'http://127.0.0.1:8000/audio/output.wav'
#        response = urllib.request.urlopen(url)
#        data = response.read() 
#        signal = wave.open(data)
# 'H' is for unsigned 16 bit integer, try 'h' also
#        sample_buff = struct.unpack('h', data[0:2])
        f0 = pitchTrackingYIN(ROOT+'/SpeechBuddy/audio/output.wav', (40, 300), 0.1, 0.25, 44.1e3, 1e3)
